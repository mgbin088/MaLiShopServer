...
class cVI_BASE(cVIEWS_ad):
    ...
    def getBreadcrumb(self):
        ...
        if self.sub1id:
            ...
            if menu2:
                breadcrumb += ('<li style="float:left;"><a href="/%s/%s">%s</a></li>' %
    ...
    def showMsgLink(self, dR):
        ...
        if str(R) == '1':  # 错误
            ...
            if dR.get('isadd', '') != '':
                self.pageNo = 1
            url = '/%s/%s/?pageNo=%s' % (self.classpath, self.viewid, self.pageNo)
    ...
    def memotest(self):
        ...
        # def hiddendata(self):
        #     user = {}
        #     user['usr_id'] = self.biz.usr_id
        #     user['usr_name'] = self.biz.usr_name
        #     html_hidden = ''
        #     for h in self.hidden.items():
        #         html_hidden += '<input type="hidden" name="%s" value="%s" />' % (h[0], h[1])
        #     mainData = {'headerJs': self.include_script(self.js), 'headerCss': self.include_css(self.css),
        #                 'sTitle': self.sTitle, 'site_name': self.site_name, 'navTitle': self.navTitle
        #         , 'localurl': self.biz.localurl, 'session_user': user, 'pageNo': self.biz.pageNo, 'backUrl': self.backUrl
        #         , 'cookieprefix': self.biz.cookie.keyname, 'sUrl': self.sUrl, 'pageNo': self.pageNo,
        #                 '__flag': self.biz.save_flag
        ...
        #         , 'system_menu': self.biz.system_menu
        #         , 'requiredIcon': self.requiredIcon
        #         , 'off_id': self.biz.of_id, 'unit_id': self.biz.unit_id
        #         , 'inclass': self.inclass
        #                 }
        #     return mainData
        #def self.specialinit():
        #    pass
